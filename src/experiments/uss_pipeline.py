...
class NeuralLambdaModel(torch.nn.Module):
    def __init__(self, config):
        ...
        self.transformer = torch.nn.TransformerEncoder(
            torch.nn.TransformerEncoderLayer(d_model=config.MODEL_DIM, nhead=config.NUM_HEADS, batch_first=True),
            num_layers=12
        )
...
def run_experiment():
    ...
    print(f"Target Hardware SM89 Utilization: Optimized via Custom Triton Kernel")
